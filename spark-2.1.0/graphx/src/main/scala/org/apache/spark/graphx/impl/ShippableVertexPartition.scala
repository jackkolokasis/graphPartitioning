...
class ShippableVertexPartition[VD: ClassTag](
  ...
  def get_VertexPartitionOutEdges(pid: PartitionID): Int = {
    this.routingTable.get_outEdges(pid)
  }

  def get_VertexPartitionInEdges(pid: PartitionID): Int = {
    this.routingTable.get_inEdges(pid)
  }

  def getIndex : VertexIdToIndexMap = {index}

  def getValues : Array[VD] = {values}

  def getMask : BitSet = {mask}

  def getRoutingTable : RoutingTablePartition = {routingTable}
