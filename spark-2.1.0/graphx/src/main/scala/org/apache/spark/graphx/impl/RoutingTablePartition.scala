...
class RoutingTablePartition(
 ...
 def get_outEdges(partID: PartitionID): Int = {
   var i = 0
   var outEdges = 0
   for {(arr, src, dst) <- routingTable} {
    if (partID != i) {
       outEdges += arr.length
     }
     i += 1
 ...
 def get_inEdges(partID: PartitionID): Int = {
   var i = 0
   var inEdges = 0
   for {(arr, src, dst) <- routingTable} {
     if (partID == i) {
 ...
 def getValue: Array[(Array[VertexId], BitSet, BitSet)] = {routingTable}
