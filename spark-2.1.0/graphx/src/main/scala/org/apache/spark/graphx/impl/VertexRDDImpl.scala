...
class VertexRDDImpl[VD] private[graphx] (
  ...
  override def withEdges(edges: EdgeRDD[_]): VertexRDD[VD] = {
    ...
    val new_Vertex = this.withPartitionsRDD(vertexPartitions)
    // new_Vertex.printVertex()
    new_Vertex
  }

  override private[graphx] def withPartitionsRDD[VD2: ClassTag](
      partitionsRDD: RDD[ShippableVertexPartition[VD2]]): VertexRDD[VD2] = {
       val new_Vertex = new VertexRDDImpl(partitionsRDD, this.targetStorageLevel)
       new_Vertex
  }
  ...
  override def VertexGoSameEdPartition(): Int = {
    var sum = 0
    val vId2SamePartId = partitionsRDD.mapPartitionsWithIndex(
      (index: Int, it: Iterator[ShippableVertexPartition[VD]]) =>
        it.map(vertexPart => vertexPart.get_VertexPartitionInEdges(index))).map(
          _.toInt).reduce(_ + _)

    vId2SamePartId
  }

  override def VertexGoOtherEdPartition(): Int = {
    var sum = 0
    val vId2otherPartId = partitionsRDD.mapPartitionsWithIndex(
      (index: Int, it: Iterator[ShippableVertexPartition[VD]]) =>
        it.map(vertexPart => vertexPart.get_VertexPartitionOutEdges(index))).map(
          _.toInt).reduce(_ + _)
