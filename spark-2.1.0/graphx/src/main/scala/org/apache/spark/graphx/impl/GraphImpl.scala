...
class GraphImpl[VD: ClassTag, ED: ClassTag] protected (
  ...
  /* Naive partitionBy from Vanilla Spark */
...
/* Import partitionBy */
  override def partitionBy(partitionStrategy: PartitionStrategy,
    numPartitions: Int, ThreshHold: Int = 70): Graph[VD, ED] = {
    ...
    var newEdges = partitionStrategy match {
      ...
      case PartitionStrategy.HybridCutPlus => {
        ...
        val LookUpTable0 = edges
          .map(e => (e.dstId, (e.srcId, e.attr))).join(this.degrees.map(e => (e._1, e._2)))
