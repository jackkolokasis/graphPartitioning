...
class EdgePartition[
  ...
  def printClass() {
     var i = 0
     var j = 0
     println("Local Vertex SrcId :")
     while (i < this.localSrcIds.length) {
         println(srcIds(i))
     }
     println("Local Vertex DestId :")
     while (j < this.localDstIds.length) {
         println(dstIds(j))
     }
 }
 /* Jack Kolokasis Inserted Code */
  ...
  /* Jack Kolokasis Inserted Code */
  def getSrcIds : Array[Int] = {localSrcIds}

  def getDstIds : Array[Int] = {localDstIds}
  ...
  /** Jack Kolokasis Print */
  def printEdgesInPartition(pid: PartitionID): Unit = {
    println("LocalSrcIds  LocalDstIds SrcId DstId attr")
    var i = 0
     while (i < size) {
      val localSrcId = localSrcIds(i)
      val localDstId = localDstIds(i)
      ...
      val dstId = local2global(localDstId)
      val attr = data(i)
      println("pId=" + pid + " LocalSrcId=" + localSrcId + " LocalDstId="
        + localDstId + " SrcId=" + srcId + " DstId=" + dstId)
      i += 1
    }
  }
  /**
