...
abstract class VertexRDD[VD](
  ...
  /* Jack Kolokasis Inserted Code to Print Vertex */
  def VertexGoSameEdPartition(): Int

  def VertexGoOtherEdPartition(): Int

  /* Jack Kolokasis Inserted Code */
...
object VertexRDD {
  ...
  def fromEdges[VD: ClassTag](
    ...
    val vert = new VertexRDDImpl(vertexPartitions)
    vert
  }
...
//    val tmp1 = vid2pid
//    val part = vid2pid.partitions
//    for (p <- part) {
//      val idx = p.index
//      val partRdd = tmp1.mapPartitionsWithIndex{(partId, msg) =>
//      if (partId == idx) msg else Iterator()}
//    val data = partRdd.collect
//    for (d <- data) {
//      println("PartitionId: " + idx + " elements : " + "[ " + d.toString + "]")
//    }
// }

    val numEdgePartitions = edges.partitions.length
//    println("NumEdgePartitions = " + numEdgePartitions)
    val tmp = vid2pid.partitionBy(vertexPartitioner).mapPartitions(
      iter => Iterator(RoutingTablePartition.fromMsgs(numEdgePartitions, iter)),
      preservesPartitioning = true)
//    var i = 0
//
//    val part2 = tmp.partitions
//    for (p <- part2) {
//      val idx = p.index
//      val partRDD = tmp.mapPartitionsWithIndex{(partId, partMsg) =>
//        if (partId == idx) partMsg else Iterator()}
//      val data = partRDD.collect
//      for (d <- data) {
//        for {(arr, src, dst) <- d.getValue} {
//          println("PartitionId: " + idx + " RoutingTable-Array" +
//            i.toString + " " + arr.deep.mkString(" ").toString)
//
//          var j = 0
//          var strSrc : String = null
//          var strDst : String = null
//
//          while (j < src.capacity) {
//            strSrc += src.get(j).toString
//            strSrc += " "
//            j+=1
//          }
//
//          j = 0
//          while(j < dst.capacity) {
//            strDst += dst.get(j).toString
//            strDst += " "
//            j+=1
//          }
//
//          println("PartitionId: " + idx + " Src: " + i.toString + " " + strSrc)
//          println("PartitionId: " + idx + " Dst: " + i.toString + " " + strDst)
//          i+=1
//        }
//      }
//    }
